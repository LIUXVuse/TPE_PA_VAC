...
「快樂署北PA假日抽前系統」是一個專為小型團隊設計的網頁應用程式，旨在簡化和公平化連續假期的排班流程。系統提供兩種不同的排班模式，以應對不同性質的假期需求：一種是基於權重優先級的申請模式，另一種是結合自願登記與隨機抽籤的特殊模式。所有資料皆透過 Cloudflare KV 雲端儲存，實現跨裝置同步。
...
-   **自動分配假期**:
    -   提供「**自動分配剩餘假期**」按鈕。
    -   點擊後，系統會掃描所有**一般模式**假期，將剩餘名額優先分配給**年度休假最少的成員**。
