...
const HolidayManager: React.FC<HolidayManagerProps> = ({ holidays, selectedHolidayId, onAddHoliday, onRemoveHoliday, onSelectHoliday }) => {
      ...
      {isFormVisible && (
        <form onSubmit={handleAddHoliday} className="mt-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} required className="w-full bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} required className="w-full bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
      ...
      <div className="mt-6 space-y-2 max-h-60 overflow-y-auto pr-2">
        {holidays.length > 0 ? holidays.map(holiday => (
          <div
            ...
            className={`flex justify-between items-center p-3 rounded-md cursor-pointer transition-colors duration-200 ${selectedHolidayId === holiday.id ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-700 hover:bg-gray-600'}`}
          >
            <div>
