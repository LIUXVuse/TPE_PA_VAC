...
const HolidayOverview: React.FC<HolidayOverviewProps> = ({ holidays, teamMembers }) => {
  const overviewData = useMemo(() => {
    const data: Map<string, { memberName: string; approvedHolidays: ApprovedHoliday[] }> = new Map();

    teamMembers.forEach(member => {
      data.set(member.id, { memberName: member.name, approvedHolidays: [] });
    });
      ...
      if (holiday.isSpecialLottery) {
        holiday.dailyAssignments?.forEach(assignment => {
          ...
          if (memberData) {
            ...
            memberData.approvedHolidays.push({
    ...
    return Array.from(data.values()).sort((a, b) => b.approvedHolidays.length - a.approvedHolidays.length);
  ...
  return (
    <div className="mt-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {overviewData.map(({ memberName, approvedHolidays }) => (
          <div key={memberName} className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg flex flex-col">
            <div className="flex justify-between items-baseline mb-4">
              ...
              <span className="text-lg font-semibold text-blue-400 bg-blue-900/50 px-3 py-1 rounded-full">
                {approvedHolidays.length} 天
              </span>
            </div>
            <div className="flex-grow space-y-2 overflow-y-auto max-h-48 pr-2 custom-scrollbar">
              {approvedHolidays.length > 0 ? (
                approvedHolidays.map((h, index) => (
                  <div key={index} className="bg-gray-700 p-2.5 rounded-md text-sm">
                    <p className="font-semibold text-gray-200">{h.holidayName}</p>
                    <p className="text-xs text-gray-400">{h.dateInfo}</p>
                  </div>
                ))
              ) : (
                <div className="h-full flex items-center justify-center">
                    <p className="text-gray-500 text-sm">今年尚無已批准假期。</p>
                </div>
              )}
            </div>
          </div>
